...
import { CellProtectionModelType } from "../../Sequelize/Models/CellProtection";
...
export async function loadSheetData(req: Request, res: Response) {
    try {
        ...
        for (let i = 0; i < sheets!.length; i++) {
            ...
            // 10. 查询单元格保护数据
            await parseProtect(worker_sheet_id, currentSheetData);
...
/**
 * parseCellProtect 解析单元格保护
 */
async function parseProtect(worker_sheet_id: string, currentSheetData: WorkerSheetItemType) {
    // 查询当前 sheet Index 的单元格保护数据，并将其封装为 Array<{range:string}> 的格式
    try {
        const result: CellProtectionModelType[] = [];
        const charts = await CellProtectionService.findCellProtection(worker_sheet_id);
        charts?.forEach((chart) => {
            const data = chart.dataValues;
